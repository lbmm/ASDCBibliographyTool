refeered = "http://adsabs.harvard.edu/cgi-bin/nph-abs_connect?" \
           "db_key=AST&db_key=PHY&db_key=PRE&qform=AST&" \
           "arxiv_sel=astro-ph&arxiv_sel=cond-mat&arxiv_sel=cs&" \
           "arxiv_sel=gr-qc&arxiv_sel=hep-ex&arxiv_sel=hep-lat&" \
           "arxiv_sel=hep-ph&arxiv_sel=hep-th&arxiv_sel=math&" \
           ...
           "&aut_xct=YES&aut_logic=SIMPLE&obj_logic=OR&" \
           "author=%s&" \
           "object=&" \
           "start_mon=%s&start_year=%s&end_mon=%s&end_year=%s" \
           "&ttl_logic=OR&title=&txt_logic=OR&text=&" \
           "start_nr=1&jou_pick=NO&ref_stems=&data_and=ALL" \
           "&group_and=ALL&start_entry_day=&start_entry_mon=&" \
           "start_entry_year=&end_entry_day=&end_entry_mon=&" \
           "end_entry_year=&min_score=&sort=SCORE&data_type=PLAINTEXT&" \
...
not_refeered = "http://adsabs.harvard.edu/cgi-bin/nph-abs_connect?" \
               "db_key=AST&db_key=PHY&db_key=PRE&qform=AST&arxiv_sel=astro-ph" \
               "&arxiv_sel=cond-mat&arxiv_sel=cs&arxiv_sel=gr-qc" \
               "&arxiv_sel=hep-ex&arxiv_sel=hep-lat&arxiv_sel=hep-ph" \
               "&arxiv_sel=hep-th&arxiv_sel=math&arxiv_sel=math-ph&arxiv_sel=nlin" \
               "&arxiv_sel=nucl-ex&arxiv_sel=nucl-th&arxiv_sel=physics" \
               "&arxiv_sel=quant-ph&arxiv_sel=q-bio&sim_query=YES&ned_query=YES" \
               "&adsobj_query=YES&aut_xct=YES&aut_logic=OR&obj_logic=OR" \
               "&author=%s" \
               "&object=&start_mon=%s&start_year=%s&end_mon=%s&end_year=%s" \
               "&ttl_logic=OR&title=&txt_logic=OR" \
               "&text=&nr_to_return=200&start_nr=1&jou_pick=EXCL&ref_stems=" \
